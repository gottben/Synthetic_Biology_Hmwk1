...
def change_rbs(rprt, target, dict_of_circuit):
    # min(on_threshold,off_threshold) < lower_bound, the other one has to be >
    # upper_bound ll
...
def change_promoter(rprt, target, dict_of_circuit):
    ...
    x = find_optimal_x(rpr, target, ('ymax', 'ymin'), 0,
                       upper_bound, lower_bound, init_x)
...
def stretch(rprt, target, dict_of_circuit):
    ...
    init_x = (ymin_x + ymax_x) / 2
    x = find_optimal_x(rpr, target, ('ymax', 'ymin'), 1,
                       upper_bound, lower_bound, init_x)
...
def does_signal_match(rpr, dict_of_circuit):
    ...
    # Turns list like [(1,5),(5,10),(2,7)] ---> [(1, 5, 2), (5, 10, 7)]
    zipped_bounds = list(zip(*bounds))
    # min(on_threshold,off_threshold) < lower_bound, the other one has to be >
    # upper_bound
    lower_bound, upper_bound = max(zipped_bounds[0]), min(zipped_bounds[1])
...
def func(x, rpr, target, param, stretch):
    ...
    if not stretch:
        # param will either be a string (ex: 'n' or 'k') or a tuple (ex:
        # ('ymax','ymin'))
        for p in param:
...
# Finds the optimal x value that allows the rpr parameter to get as close
# to the target as possible w/o changing how it affects the other ones in
# the circuit
def find_optimal_x(rpr, target, param, stretch, upper_bound, lower_bound, init_x=1):
    cons = ({'type': 'ineq', 'fun': lambda x, rpr, lower_bound, param, stretch: on_threshold(copy.deepcopy(rpr), x[0], param, stretch) - lower_bound, 'args': (rpr, lower_bound, param, stretch)},
            {'type': 'ineq', 'fun': lambda x, rpr, upper_bound, param, stretch: upper_bound -
                off_threshold(copy.deepcopy(rpr), x[0], param, stretch), 'args': (rpr, upper_bound, param, stretch)},
            {'type': 'ineq', 'fun': lambda x: x})

    res = minimize(func, np.array([init_x]), args=(
        rpr, target, param, stretch), constraints=cons, method='COBYLA')
